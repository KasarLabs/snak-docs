...
The **Starknet Agent Kit** now follows a decentralized approach where each contributor can add their own implementation for a specific protocol. This reorganization makes it easier to understand and scale the project by clearly isolating the logic for each protocol.

For core requests—such as RPC methods or shared utilities—please add them under:

```
lib/agent/methods/core/
```
...
### Implementation Library (`lib/agent/methods/`)
